...
void FusedPointcloud::setNewAnchor(
    const pcl::PointCloud<pcl::PointXYZRGB>::Ptr pcl_cloud_anchor,
    const Eigen::Matrix4d& T_O_S_anchor, const uint64_t kf_id_anchor,
...
void FusedPointcloud::addPCLCloud(
  ...
  *fused_pcl_cloud_ += transformed_cloud;
  ...
  // pcl::PointCloud<pcl::PointXYZRGB>::Ptr fused_pcl_cloud_filtered(new
  // pcl::PointCloud<pcl::PointXYZRGB>); pcl::VoxelGrid<pcl::PointXYZRGB> sor;
...
void FusedPointcloud::filterFusedPCLCloud() {
  auto start = std::chrono::steady_clock::now();
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr fused_pcl_cloud_filtered(
